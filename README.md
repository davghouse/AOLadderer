...
Latest release [here](https://github.com/davghouse/AOLadderer/releases/tag/v2.0.0).

![Build](/Screenshots/Build.png)
![Basic Ladder](/Screenshots/2.png)
![Basic Shopping](/Screenshots/3.png)
![Advanced Ladder](/Screenshots/4.png)
![Advanced Shopping](/Screenshots/5.png)
...
| Slot | QL  | Ability Req      | Treatment Req. | Shiny         | Bright         | Faded, Val. |
| ---- | --- | ---------------- | -------------- | ------------- | -------------- | ----------- |
...
Earlier version of Windows aren't supported but there's a chance it might work on them.
...
Don't remove ladder implants until absolutely necessary.
...
The first strategy uses no ladder implants, just equipping required implants in the order which maximizes their average QL (this is non-obvious).
...
The average QL from the tool is slightly higher, but the Matter Crea is 2 points less, which could be a big deal.
...
This swap is so common the tool supports it directly.
Just remember, if you use this swap you can't use Treatment buffing weapons like the Quartz rifle.
There's no way to tell the tool (yet) that sometimes you have +10 Treatment and +8 Sta, and other times you have +20 Str/Sta/Agi/Sen/Int/Psy.
...
This combined buff is supported by the tool.

**You'll have to tell the tool about your ability to swap items (other than Explosifs), by manually increasing your starting abilities.
...
Another tip: if L/R wrists and left-hand are important for your setup, consider using the tool on those three alone, and then doing the rest of your implants.
This is OK because those aren't ladder slots, so the tool will never use them when laddering up the rest.
...
Get the set of ladder implants found by the basic solver, and then ladder those implants up via the basic solver (and then uses those laddered ladder implants to equip the final implants).
Going any further than one level deep isn't worth it, we very quickly run into diminishing returns.
...
the final implants, given the ladder implants we've found so far (which get unequipped and replaced by final implants in the base solver).
